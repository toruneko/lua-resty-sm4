...
--- config
    location = /t {
        content_by_lua_block {
            ...
            local bytes = sm4:encrypt_block({0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF, 0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10})
            local data = {}
            for i = 1, 16, 1 do
...
--- config
    location = /t {
        content_by_lua_block {
            local resty_sm4 = require "resty.sm4"
            local sm4 = resty_sm4:new({0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF, 0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10})
            local bytes = sm4:decrypt_block({0x68, 0x1e, 0xdf, 0x34, 0xd2, 0x06, 0x96, 0x5e, 0x86, 0xb3, 0xe9, 0x4f, 0x53, 0x6e, 0x42, 0x46})
            local data = {}
            for i = 1, 16, 1 do
                data[i] = string.format("%02x", bytes[i])
            end
